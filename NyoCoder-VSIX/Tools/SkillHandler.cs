...
{
    /// <summary>
    /// Discovers and manages agent skills under %APPDATA%\NyoCoder\Skills.
    /// Each skill is a folder containing SKILL.md (YAML frontmatter + markdown body).
    /// </summary>
    internal static class SkillHandler
    {
        ...
        /// <summary>
        /// Fixed skills root: %APPDATA%\NyoCoder\Skills
        /// </summary>
        ...
        /// <summary>
        /// Creates the skills directory if it does not exist. Returns the absolute path.
        /// </summary>
        public static string EnsureSkillsDirectory()
        {
        ...
        /// <summary>
        /// True when at least one built-in skill tool is enabled for the given mode.
        /// </summary>
        public static bool AnySkillToolEnabled(IEnumerable<string> enabledTools)
        {
            if (enabledTools == null)
                return false;
            foreach (string name in enabledTools)
            {
                if (string.Equals(name, "view_skill", StringComparison.OrdinalIgnoreCase)
                    || string.Equals(name, "create_skill", StringComparison.OrdinalIgnoreCase)
                    || string.Equals(name, "edit_skill", StringComparison.OrdinalIgnoreCase)
                    || string.Equals(name, "edit_skill_file", StringComparison.OrdinalIgnoreCase)
                    || string.Equals(name, "remove_skill", StringComparison.OrdinalIgnoreCase))
                {
                    return true;
                }
            }
        ...
        public static string GetContext()
        {
            ...
            {
                bool unused;
                sb.AppendLine(skill.FolderName + ": " + TruncateDescription(skill.Description, out unused));
            }
            return sb.ToString().Trim();
        }
        ...
        public static string CreateSkill(string name, string description, string instructions)
        {
            ...
            string root = EnsureSkillsDirectory();
        ...
        public static string EditSkillFile(string name, string relativePath, string content)
        {
            ...
            string displayRelative;
            string full = ResolveSkillRelativePath(existing, relativePath, out displayRelative);
        ...
        {
            string displayRelative;
            string full = ResolveSkillRelativePath(skill, relativePath, out displayRelative);
            if (!File.Exists(full))
