...
public static class ToolHandler
{
    ...
    /// <summary>Unified result fields for built-in and external tools (text + optional image).</summary>
    ...
    public static bool ExecuteToolCall(ToolCall call, string modeId, out string toolContent, out int exitCode)
    {
        string imageBase64;
        string imageMime;
        return ExecuteToolCall(call, modeId, out toolContent, out exitCode, out imageBase64, out imageMime);
    }
        ...
        try
        {
            ...
            switch (call.Name)
            {
                ...
                default:
                    ...
                    {
                        ExternalToolRegistry.ExecuteToolCall(call.Name, call.Arguments,
                            out toolContent, out exitCode, out imageBase64, out imageMime);
                        return true;
                    }
    ...
    /// <summary>
    /// JSON-first parse of external tool stdout. On failure, entire stdout is plain text.
    /// </summary>
    public static void ParseToolStdout(string stdout, out string text, out string imageBase64, out string imageMime)
    {
        text = stdout ?? "";
        imageBase64 = null;
        imageMime = null;
